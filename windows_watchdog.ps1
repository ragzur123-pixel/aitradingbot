...
Write-Host "--- INITIALIZING WINDOWS WATCHDOG ---" -ForegroundColor Cyan

while($true) {
    # 1. Check if Ollama is running
    $ollama = Get-Process ollama -ErrorAction SilentlyContinue
    if (!$ollama) {
        Write-Host "[WARNING] Ollama is down. Restarting..." -ForegroundColor Yellow
    ...
    if (!$bot) {
        Write-Host "[WARNING] Trading Bot is down. Restarting..." -ForegroundColor Yellow
